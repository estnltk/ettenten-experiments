...
#          Aligns Vabamorf's morph analysis layer and EWTB's morph analysis 
#          (ud_syntax) layer: for each Vabamorf's analysis, finds out 
#          whether it matches with the EWTB's morph analysis.
#          By deafult, uses align_records() for comparing annotations.
#          Outputs alignment results (matches/mismatches) as a layer.
#
...
class VM2UDMorphFullDiffTagger(Tagger):
    """Creates alignments between Vabamorf's morph analysis layer and ud_syntax morph analysis layer."""
    
    conf_param = ['vm_morph_layer', 'ud_syntax_layer', 'compare_function', 'count_mismatch_details']
    output_attributes = ('root_match', 'pos_match', 'form_match', 'full_match')
